...
<style>
            ...
            <div class="container-fluid">
                <div class="row">
                                ...
                                <div class="col-xl-12">
                                    <div class="card">
                                        <div class="card-header">
                                            <h6 class="card-title m-0">Test Organism</h6><a class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add-sample-type">Add Test Organism</a>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <table id="organism-setup" class="table card-table table-hover align-middle mb-0" style="width: 100%;">
                                                <thead>
                                                    <tr>
                                                        <th style="width:3%">#</th>
                                                        <th style="width:90%">Organism</th>
                                                        <th style="width:7%"class="text-center">Actions</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                @foreach($organism as $key => $value)
                                                    <tr>
                                                        <td>{{ ++$key }}</td>
                                                        ...
                                                        <td class="text-center">
                                                            ...
                                                            <a href="{{ 'block-organism-setup/'.$value->id }}"  onclick="return confirm('Do you want to block this test organism?')" class="btn btn-link btn-sm text-info" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Active" aria-label="Active">
                                                                <i class="fa fa-ban" style="color:#198754;"></i>                                                                
                                                            </a>
                                                            <?php } else { ?>
                                                                <a href="{{ 'unblock-organism-setup/'.$value->id }}" onclick="return confirm('Do you want to unblock this test organism?')" class="btn btn-link btn-sm text-danger" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Blocked" aria-label="Blocked">
                    ...
                    @endforeach
                            ...
                            <form class="row g-3" id="addorganism" name="addorganism" method="post" action="{{ route('organism.add') }}">                            
                                @csrf
                                <div class="modal fade" id="add-sample-type" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog modal-lg modal-dialog-scrollable">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title">Test Organism</h5>
                                                <button type="button" id="add-modal-close-x" class="btn-close"></button>
                                            </div>
                                            <div class="modal-body custom_scroll">
                                                <div class="row g-4">
                                                    <div class="col-sm-12">
                                                        <label class="form-label">Test Organism Name <span class="red">*</span></label>
                                                        <input type="text" class="form-control form-control-lg" placeholder="Enter here" name="organismname" id="organismname" >
                                                        <div class="alert-danger pt-1 pb-1 px-1 py-1" id="organismnamealert" style="display:none ;">Please enter test organism </div>                                                
                                                    </div>                                    
                                                </div>                                
                                            </div>
                                            <div class="modal-footer">
                                                <a class="btn btn-primary" onclick="addorganismValidation();">Save</a>
                            ...
                            <form class="row g-3" name="editorganism" method="post" action="{{ route('organism.edit') }}">                            
                                @csrf
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body custom_scroll">
                                                <div class="row g-4">
                                                    <div class="col-sm-12">
                                                        <label class="form-label">Test Organism Name <span class="red">*</span></label>
                                                        <input type="text" class="form-control form-control-lg" placeholder="Enter here" name="organismnameedit" id="organismnameedit" >
                                                        <input type="hidden" class="form-control form-control-lg" name="hiddenid" id="hiddenid"  >
                                                        <div class="alert-danger pt-1 pb-1 px-1 py-1" id="organismnamealertedit" style="display:none ;">Please enter test organism </div>                                                
                                                    </div>                                    
                                                </div>                                
                                            </div>
                                            <div class="modal-footer">
                                                <a class="btn btn-primary" onclick="editorganismValidation();">Save</a>
    ...
    $('#add-modal-close-x, #add-modal-close').click(function() {
        $('#organismname').val('');
        $('#organismnamealert').hide();
        $('#add-sample-type').modal('hide');
    });

    $('#edit-modal-close-x, #edit-modal-close').click(function() {
        $('#organismnamealertedit').hide();
        $('#edit-sample-type').modal('hide');
    });

   
    function addorganismValidation() {
        document.getElementById('organismnamealert').style.display = 'none';
        if(document.addorganism.organismname.value == ''){
            document.getElementById('organismnamealert').style.display = 'block';  
            document.addorganism.organismname.focus(); 
            return false;     
        }
    document.addorganism.submit();
    return true;
    }
    ...
    function editview(id){
        ...
        $.ajax({
            ...
            success: function(response){
                $('#organismnameedit').val(response.organism);
    ...
    function editorganismValidation() {
        if(document.editorganism.organismnameedit.value == ''){
            document.getElementById('organismnamealertedit').style.display = 'block';  
            document.editorganism.organismnameedit.focus(); 
            return false;     
        }
    document.editorganism.submit();
    return true;
    }
    </script>
